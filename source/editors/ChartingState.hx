...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Move Camera When Singing', "Value 1: Type the offset value of the camera\nwhen a character sings (leave it blank to apply\nthe default value that is 25).\n\nThis event does nothing if the \"Move Camera\nwhen Sing\" option is unchecked."],
	...
	**/
	...
	override function create()
	{
		...
		//FlxG.save.bind('funkin', 'Saved Data');
	...
	function addSongUI():Void
	{
		...
		//this shit is from vs zRamirez
		var loadCamEventJson:FlxButton = new FlxButton(110, loadEventJson.y, 'Load Cam Events', function()
			{
		...
		var loadSawsEventJson:FlxButton = new FlxButton(loadEventJson.x, loadEventJson.y + 23, 'Load Saws Events', function()
			{
				var songName:String = Paths.formatToSongPath(_song.song);
				var file:String = Paths.json(songName + '/saws-events');
				#if sys
				if (#if MODS_ALLOWED FileSystem.exists(Paths.modsJson(songName + '/saws-events')) || #end FileSystem.exists(file))
				#else
				if (OpenFlAssets.exists(file))
				#end
				{
					clearEvents();
					var events:SwagSong = Song.loadFromJson('saws-events', songName);
					_song.events = events.events;
					changeSection(curSection);
				}
			});
		
			loadSawsEventJson.setGraphicSize(80, 30);
			loadSawsEventJson.updateHitbox();
		
			//ayuda estoy sufriendo añadiendo esta linea de codigo, por favor no me vuelvan a pedir más updates del qt fucked o les juro que me suicidare
		...
		tab_group_song.add(loadCamEventJson); //more shit from vs zramirez bruhhhhhh
		tab_group_song.add(loadSawsEventJson);
	...
	function addEventsUI():Void
	{
		...
		var directories:Array<String> = [Paths.getPreloadPath('events/'), Paths.mods('custom_events/'), Paths.mods(Paths.currentModDirectory + '/custom_events/')];
		for (i in 0...directories.length) {
			var directory:String =  directories[i];
			if(FileSystem.exists(directory)) {
				for (file in FileSystem.readDirectory(directory)) {
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput && !fuckingCheater)
		{
			...
			if (FlxG.keys.justPressed.TAB) //Fixed the tabbing shit by Drkfon <--- mentira lo arreglo zramirez deah
				{
					if (FlxG.keys.pressed.SHIFT)
