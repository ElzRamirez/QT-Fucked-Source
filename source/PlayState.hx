...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['NIGGER!!!!!!!', 0.2], //From 0% to 19% //no me funen plis
		['MAN WHATS HAPPEN WITH YOU?', 0.4], //From 20% to 39%
		['YOU REALLY SUCK', 0.5], //From 40% to 49%
		['YOU SUCK', 0.6], //From 50% to 59%
		['MAN YOU SUCK', 0.69], //From 60% to 68%
		['...', 0.7], //69%
		['OK', 0.8], //From 70% to 79%
		['FUCK', 0.9], //From 80% to 89%
		['FUCKED!', 1], //From 90% to 99%
		['TOUCH GRASS!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'censory-fuckedload' | 'fuckedminate':
					curStage = 'street-kb';
				case 'fuckedmination':
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'fuckinfree':
					FlxG.sound.playMusic(Paths.music('CuteShit'), 0);
					FlxG.sound.music.fadeIn(1, 0, 0.75);
					startDialogue(dialogueJson);

				case 'fuckedless' | 'fuckedminate':
					startDialogue(dialogueJson);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			...
			if(endingSong) {
				...
				if(SONG.song.toLowerCase()== 'fuckinfree' || SONG.song.toLowerCase()=='censory-fuckedload'){
	...
	function startSong():Void
	{
		...
		}else if (SONG.song.toLowerCase() == 'censory-fuckedload') {
			gfSpeed = 2;
		...
		if(Paths.formatToSongPath(SONG.song) == 'fuckedmination' && storyDifficulty==99){
			trace("USING FUCKEDMINATION OLD INST");
		...
		if(SONG.song.toLowerCase() == "fuckedminate") //Fakes the timebar
			songLength=316219;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if(Paths.formatToSongPath(SONG.song) == 'fuckedmination' && storyDifficulty==2){
			trace("USING FUCKEDMINATION OLD INST (dafuq)");
        ...
        var file3:String = Paths.json(songName + '/saws-events');
        #if sys
        if (FileSystem.exists(Paths.modsJson(songName + '/saws-events')) || FileSystem.exists(file3)) {
        #else
        if (OpenFlAssets.exists(file3)) {
        #end
            var sawsEventsData:Array<Dynamic> = Song.loadFromJson('saws-events', songName).events;
            for (event in sawsEventsData) //An another optional extra json to separately read KB_Alert or KB_AttackFire events or others if you want
            {
                for (i in 0...event[1].length)
                {
                    var newEventNote:Array<Dynamic> = [event[0], event[1][i][0], event[1][i][1], event[1][i][2]];
	...
	override public function update(elapsed:Float)
	{
		...
		healthLerp = FlxMath.lerp(health, healthLerp, Math.exp(-elapsed * 9));
	...
	function kbATTACK_ALERT_PART2(newAlpha:Float, animationToPlay:String, alertminationTrueAlert:Null<Bool> = false):Void{
		...
		switch(animationToPlay){
			default:
				...
				{
					if (alertminationTrueAlert){
						if (ClientPrefs.alertSkin == 'Extreme'){
				...
				if (kb_attack_alert != null)
				{
					if (alertminationTrueAlert){
						if (ClientPrefs.alertSkin == 'Extreme'){
							...
							kb_attack_alert.scale.set(1.5,1.5);
							kb_attack_alert.offset.set(35,5);
						}
					}
					else if (ClientPrefs.alertSkin == 'Extreme'){
						kb_attack_alert.offset.set(0,0);
				...
				if(!bfDodging){
					if(!godMode){
						...
						if(maxSawbladeHits == 0 || (maxSawbladeHits == 4 && sawbladeHits > 3) || (maxSawbladeHits == 8 && sawbladeHits > 7)){
							...
							else if (maxSawbladeHits == 8)
							{
								health -= 0.145;
								if(health >= maxHealth+0.25625){ //Almost the half of the original -DrkFon
									maxHealth = maxHealth+0.25625;
									reduceMaxHealth();
									//Only reduce max health if possible. This is here to hopefully avoid a crash with the regerneating the bar or something.
								}
							}
			...
			case 'Screen Shake':
				...
				for (i in 0...targetsArray.length) {
					...
					if(duration > 0 && intensity != 0) {
						...
						if (targetsArray[i] == camHUD)
							camPincers.shake(intensity, duration);
					}
				}
	...
	function startEndingDialogue(){
		if((isStoryMode || SONG.song.toLowerCase()=="cessation") && !endingCutsceneDone){
			...
			if(SONG.song.toLowerCase()=="fuckedminate" && storyDifficulty==3){
	...
	public function endSong():Void
	{
		...
		if(!startingSong && Paths.formatToSongPath(SONG.song) != 'fuckedminate') {
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(((storyDifficulty >= 2 && storyDifficulty <= 5) || healthDrainSongs.indexOf(SONG.song.toLowerCase()) != -1) && dadDrainHealth>0 && !note.ignoreNote && !note.hitCausesMiss){
			if (maxSawbladeHits == 8)
			{
				...
				if(health - dadDrainHealth - 0.1 > maxHealth){
					...
					//Health drain on sustain notes. Note that this is ignored if you've been hit by 4 or more sawblades.
					if(dadDrainHealthSustain && note.isSustainNote && sawbladeHits < 4){
						switch(sawbladeHits){
								case 0 | 1:
									if(health < 1.35){
										if(health < 0.8){
			...
			{
				//OMG I'M SO FUCKING LAZY TO CHECK THIS LINE BY LINE SO I BETTER COPYPASTED THE ORIGINAL AND THAT'S IT HOLY SHIT -Drkfon
				//prevents health drain if the drain would kill the player.
				if(health - dadDrainHealth - 0.1 > maxHealth){
					//And here I thought that this code couldn't get any worse. What is wrong with me?
			...
			*/
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled && !unlock) {
				switch(achievementName)
				{
					...
					case 'ur_good':
						if(ratingPercent >= 1 && !usedPractice && Paths.formatToSongPath(SONG.song) != 'fuckedminate') {
							unlock = true;
						}
					case 'fuckedmination_beat':
