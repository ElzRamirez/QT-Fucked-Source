...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		if(CoolUtil.difficulties.length < 2 || PlayState.SONG.song.toLowerCase() == 'fuckinfree' || PlayState.SONG.song.toLowerCase() == 'fuckedless' || PlayState.SONG.song.toLowerCase() == 'censory-fuckedload' || PlayState.SONG.song.toLowerCase() == 'fuckedmination' || PlayState.SONG.song.toLowerCase() == 'fuckedmination-vip' || PlayState.SONG.song.toLowerCase() == 'fuckedmination-duet-vip' || PlayState.SONG.song.toLowerCase() == 'fuckedmination-corrupted' || PlayState.SONG.song.toLowerCase() == 'cessation' || PlayState.SONG.song.toLowerCase() == 'interlope')
			menuItemsOG.remove('Change Difficulty'); //No need to change difficulty if there is only one!

		if (!PlayState.songDoesNotHaveSawblades && !PlayState.cantChangeMaxSawbladeHits && !PlayState.isStoryMode)
		{
		...
		if (accepted)
		{
			...
			switch (daSelected)
			{
				...
				case 'Change Difficulty':
					...
					regenMenu();
				case 'Change Max Sawblade Hits':
					FlxG.sound.play(Paths.sound('scrollMenu'), 0.4);
					openSubState(new SelectSawbladesAmountSubState(PlayState.maxSawbladeHits, true));
